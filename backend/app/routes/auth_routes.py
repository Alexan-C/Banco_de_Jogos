...
from app.dependencies.dependencies import pegar_sessao, verificar_token
...
async def criar_conta(usuario_schema: UsuarioSchema, session = Depends(pegar_sessao)):
    ...
    if usuario:
        ...
        session.add(novo__usuario)
        session.commit()
        return {"mensagem": f"usúario cadastrado com sucesso {usuario_schema.email}"}
